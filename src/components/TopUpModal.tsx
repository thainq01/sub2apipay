            ...
            return (
              ...
              >
                <span className={['text-xs', isDark ? 'text-slate-400' : 'text-slate-500'].join(' ')}>
                  {pickLocaleText(locale, `Nạp tiền +${amount} VND`, `Balance +${amount} VND`)}
                </span>
                <span className="mt-1 text-2xl font-bold text-emerald-500">{amount.toLocaleString('vi-VN')} VND</span>
              </button>
            );
          })}
        </div>
