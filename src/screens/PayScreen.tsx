...
import { resolveLocale, pickLocaleText } from '@/lib/locale';
...
export default function PayScreen({ token, isIframe, navParams }: PayScreenProps) {
  const { theme } = useThemeStore();
  const isDark = theme === 'dark';
  const locale = resolveLocale(navParams.lang);
  const resumeOrderId = navParams.resume_order;
  const autoAmount = navParams.amount ? Number(navParams.amount) : null;
  const srcHost = navParams.src_host || undefined;
  const srcUrl = navParams.src_url || undefined;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      navigateToScreen({ ...navParams, screen: 'home' });
    }, 2200);
    return () => clearTimeout(timer);
  }, [step, finalOrderState, loadUserAndOrders, navParams]);
        ...
        <>
          ...
          <PendingOrderBanner
            ...
            buildPayUrl={(order) => {
              ...
              params.set('screen', order.orderType === 'subscription' ? 'subscriptions' : 'pay');
